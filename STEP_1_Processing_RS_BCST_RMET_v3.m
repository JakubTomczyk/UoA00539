%% This script will calculate 9 measures for all files containing RS, BCST and RMET in the folder: filepathName
...
for iFile = 1:size(myFolderInfo,1)
    ...
    writetable(tableOutput,strrep(['../OutputFiles/', computerName, ...
        '_', datestr(now, 'yyyymmdd'), '_', filename],'.RAW','_RS_BCST_RMET.xlsx'), ...
        'Sheet', 1, 'Range', 'A1')
end % loop for files
