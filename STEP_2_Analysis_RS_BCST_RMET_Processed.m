% In this script we perform EDA and statistical analysis of Processed RS, BCST and
% RMET files.


%% Analysis of files from Unprocessed_RS_BCST_RMET folder
computerName = {'pc', 'linux', 'mac'}; 
...
for iComp = 1:3
    ...
    % BCST, RMET
    fileList = dir(['../OutputFiles/Processed_RS_BCST_RMET/',...
    ...
    for jFile = 1:length(fileList(:))
        ...
        fileTask = filenameSplit{4};
        
        % Read file and add computerName, filename and resultType to table
        fileTable = readtable(['../OutputFiles/Processed_RS_BCST_RMET/',...
            filename]);
        ...
        switch fileTask(end-1:end)
            case 'rs' % rs short 5 seconds
                ...
                fileTable.Type = repmat('Processed', 1, 1);
                fileTable.Filename = {filenameSplit{4}};
    ...
    for jMeasure = 1:size(measureName, 2)
        ...
        title(strrep(['Processed_RS_BCST_RMET, ', computerName{iComp}, ', ',...
            measureName{jMeasure}], '_', '\_'))

        % Save results
        saveas(gcf, ['../Results/Processed_RS_BCST_RMET_', computerName{iComp}, '_', ...
